...
(function () {
  ...
  A.pivot = function (leads, spend, from, to, granularity, keyFn) {
    ...
    const blank = () => ({ leads: 0, qualified: 0, tbd: 0, unqualified: 0, dead: 0, visits: 0, spend: 0 });
    const finish = c => {
    ...
    /* period totals */
    const pSpend = new Map(periods.map(p => [p, 0]));
    spend.forEach(sp => {
      if (sp.date < from || sp.date > to) return;
      const p = periodOf(sp.date);
      if (pSpend.has(p)) pSpend.set(p, pSpend.get(p) + sp.cost);
    });
    ...
    /* apportion each period's spend by that period's lead share */
    rows.forEach(row => {
      periods.forEach(p => {
        const c = row.cells.get(p);
        const tot = pLeads.get(p);
        c.spend = tot > 0 ? pSpend.get(p) * (c.leads / tot) : 0;
        finish(c);
      });
    ...
    periods.forEach(p => {
      const c = blank();
      rows.forEach(r => {
        const x = r.cells.get(p);
        ['leads', 'qualified', 'tbd', 'unqualified', 'dead', 'visits', 'spend'].forEach(k => { c[k] += x[k]; });
      });
      totalRow.cells.set(p, finish(c));
    });
    ['leads', 'qualified', 'tbd', 'unqualified', 'dead', 'visits', 'spend'].forEach(k => {
      totalRow.total[k] = [...rows.values()].reduce((a, r) => a + r.total[k], 0);
    });
    finish(totalRow.total);

    return {
      periods, labels, isWeek,
      rows: [...rows.values()].sort((a, b) => b.total.leads - a.total.leads),
      totalRow
    };
  };
