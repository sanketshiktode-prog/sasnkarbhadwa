...
(function () {
  ...
  function renderPivot(ctx, isWeek) {
    ...
    const pv = A.pivot(ctx.leads, ctx.spend, ctx.range.from, ctx.range.to, S.granularity, keyFns[S.pivotDim]);
    ...
    $('#pivotTableNote').textContent = /spend|cpl|cpql/i.test(S.pivotMetric)
      ? "Spend has no per-project column in the exports, so each period's spend is apportioned by that period's lead share."
