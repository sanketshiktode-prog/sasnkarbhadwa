...
(function () {
  ...
  const SLOTS = {
    leads:   { label: 'CRM lead dump', kind: 'leads' },
    google:  { label: 'Google Ads spend', kind: 'spend', platform: 'Google' },
    meta:    { label: 'Meta Ads spend', kind: 'spend', platform: 'Meta' },
    session: { label: 'Saved session', kind: 'session' }
  };
  ...
  /* Load straight away — one slot, one file type, no staging step. */
  async function loadFiles(slot, fileList) {
    ...
    for (const f of files) {
      ...
      try {
        ...
        const detected = f.name.toLowerCase().endsWith('.json') ? 'session' : PP.detectKind(text);

        if (detected !== 'unknown' && detected !== cfg.kind) {
          logUpload(Object.assign(base, {
            status: 'Rejected',
            note: `This looks like a ${detected === 'leads' ? 'lead dump' : 'spend export'}, not a ${cfg.label.toLowerCase()}. Use the matching drop zone.`
          }));
          toast(`${f.name} does not belong in ${cfg.label}`);
          continue;
        }

        if (cfg.kind === 'leads') {
          const rows = PP.parseLeads(text, f.name);
          if (!rows.length) throw new Error('No dated lead rows found');
          ...
          if (!rows.length) throw new Error('No dated spend rows found');
          const added = mergeSpend(rows);
          const dates = rows.map(r => r.date).sort();
          logUpload(Object.assign(base, {
            rows: rows.length, added, from: dates[0], to: dates[dates.length - 1],
            note: `${A.money(A.sum(rows.map(r => r.cost)))} tagged ${cfg.platform}`
          }));
        } else {
  ...
  function bind() {
    ...
    /* one drop zone per file type — no staging, no platform dropdown to forget */
    $$('.dropzone[data-slot]').forEach(dz => {
      ...
      dz.addEventListener('drop', e => loadFiles(slot, e.dataTransfer.files));
    });
    $$('input[data-slot-input]').forEach(inp => {
      inp.addEventListener('change', e => { loadFiles(inp.dataset.slotInput, e.target.files); e.target.value = ''; });
    });
    /* a file dropped anywhere else lands in the right slot by detection */
    ...
    window.addEventListener('drop', async e => {
      ...
      goUploads();
      for (const f of files) {
        if (f.name.toLowerCase().endsWith('.json')) { await loadFiles('session', [f]); continue; }
        const text = await PP.readFile(f);
        ...
        else toast(`${f.name}: drop spend files on the Google or Meta zone so the platform is recorded`);
      }
    });
