...
(function () {
  ...
  const VIEW_META = {
    ...
    uploads: ['Uploads', 'One box per file type. Drag a file in or click the box.']
  };
  ...
  function renderDiagnostics() {
    ...
    if (!el) return;
    const rows = [
      ['CSV library (PapaParse)', typeof Papa !== 'undefined' ? 'Loaded' : 'MISSING — check access to cdn.jsdelivr.net'],
      ['Chart library (Chart.js)', typeof Chart !== 'undefined' ? 'Loaded' : 'MISSING — charts will not draw'],
      ['Upload boxes wired', DIAG.wired ? 'Yes, all four' : 'NO — reload the page'],
      ['Pickers opened / files dropped / files read', `${DIAG.picks} / ${DIAG.drops} / ${DIAG.reads}`],
      ['Last event', DIAG.lastEvent],
      ['Last error', DIAG.lastError || 'None'],
      ['In memory', `${A.n(S.leads.length)} leads · ${A.n(S.spend.length)} spend rows`]
    ];
    el.innerHTML = rows.map(([k, v]) => {
      const bad = /MISSING|^NO\b/.test(v);
      return `<div class="diagrow"><span>${esc(k)}</span><b class="${bad ? 'diagbad' : ''}">${esc(v)}</b></div>`;
    }).join('');
  }
  ...
  async function autoLoad() {
    /* Off unless the repo opts in with data-autoload="on" on <html>, so a repo
       without a data/ folder never logs a 404. */
    if (document.documentElement.dataset.autoload !== 'on') return false;
    try {
  ...
  function renderTable(mount, cols, rows, opts) {
    ...
    function view() {
      ...
      if (state.sort) {
        ...
        if (!c) { state.sort = null; return out; }   // columns changed under us
  ...
  function renderTrends(ctx) {
    ...
    renderPivot(ctx, isWeek);

    renderTable('#tblTrend', [
      { key: 'date', label: isWeek ? 'Week' : 'Date', fmt: (v, r) => (isWeek ? r.label : A.dayLabel(r.date)), sortVal: r => r.date },
  ...
  /* project (or any dimension) by day and week */
  function renderPivot(ctx, isWeek) {
    const keyFns = {
      project: l => l.project,
    ...
    const DIM = {
      ...
      asm:      ['ASM', 'All ASMs'],
      assignee: ['Owner', 'All owners']
    }[S.pivotDim];
    const dimLabel = DIM[0], allLabel = DIM[1];
    const def = A.PIVOT_METRICS[S.pivotMetric];
    /* where the campaign name identifies the row, spend is real, not apportioned */
    const match = A.projectMatcher(ctx.leads.map(l => l.project));
    const spendKeyFns = {
      project: sp => sp.project || match(sp.campaign),
      platform: sp => sp.platform,
      campaign: sp => sp.campaign || null
    };
    const pv = A.pivot(ctx.leads, ctx.spend, ctx.range.from, ctx.range.to, S.granularity,
                       keyFns[S.pivotDim], spendKeyFns[S.pivotDim] || null);
    const top = Math.max(1, Math.min(20, S.pivotTop));

    $('#pivotChartTitle').textContent = `${dimLabel} trend — ${def.label}`;
    $('#pivotChartNote').textContent = `${isWeek ? 'Weekly' : 'Daily'} · top ${Math.min(top, pv.rows.length)} of ${A.n(pv.rows.length)} by lead volume`;
    $('#pivotTableTitle').textContent = `${dimLabel} by ${isWeek ? 'week' : 'day'}`;
    $('#pivotTableNote').textContent = /spend|cpl|cpql/i.test(S.pivotMetric)
      ? (pv.matchShare >= 99.5
          ? `Spend is matched to ${dimLabel.toLowerCase()}s from the campaign names in the spend export — ${A.pct(pv.matchShare, 0)} of ${A.money(pv.totalSpend)} tied directly.`
          : `${A.pct(pv.matchShare, 0)} of ${A.money(pv.totalSpend)} matched from campaign names; the rest is apportioned by lead share in each period.`)
      : 'Straight from the CRM, no apportioning involved.';

    $('#pivotStackBox').hidden = S.pivotChart !== 'stack';
    $('#chPivotTrend').parentElement.hidden = S.pivotChart === 'stack';
    if (S.pivotChart === 'stack') C.pivotStack(pv, S.pivotMetric, top);
    else C.pivotTrend(pv, S.pivotMetric, top);

    /* flat rows so the shared table can sort, search and export them */
    const flat = [...pv.rows, pv.totalRow].map(r => {
      const o = { key: r.key === 'All' ? allLabel : r.key, __total: r.key === 'All' };
      pv.periods.forEach((p, i) => { o['p' + i] = r.cells.get(p)[S.pivotMetric]; });
      o.tot = r.total[S.pivotMetric];
      o.totLeads = r.total.leads;
      return o;
    });

    const cols = [
      { key: 'key', label: dimLabel, html: r => (r.__total ? `<b>${esc(r.key)}</b>` : esc(r.key)), csv: r => r.key },
      { key: 'tot', label: 'Window total', num: true, fmt: v => def.fmt(v) }
    ];
    pv.periods.forEach((p, i) => cols.push({
      key: 'p' + i, label: pv.labels[i], num: true,
      fmt: v => (v === null || v === undefined ? '—' : def.fmt(v))
    }));

    renderTable('#tblPivot', cols, flat, {
      sort: 'tot', dir: 'desc',
      fileName: `${S.pivotDim}_${S.pivotMetric}_by_${isWeek ? 'week' : 'day'}`,
      searchPlaceholder: `Search ${dimLabel.toLowerCase()}s…`
    });
  }
  ...
  function renderUploads() {
    const b = dataBounds();
    const note = (sel, text) => { const el = $(sel); if (el) el.textContent = text; };
    note('#upLeadsNote', S.leads.length ? `${A.n(S.leads.length)} leads · ${A.dayLabel(b.leadFrom)} – ${A.dayLabel(b.leadTo)}` : 'Nothing loaded');
    const g = S.spend.filter(x => x.platform === 'Google');
    const m = S.spend.filter(x => x.platform === 'Meta');
    note('#upGoogleNote', g.length ? `${A.money(A.sum(g.map(x => x.cost)))} · ${g.length} days` : 'Nothing loaded');
    note('#upMetaNote', m.length ? `${A.money(A.sum(m.map(x => x.cost)))} · ${m.length} days` : 'Nothing loaded');
    note('#upSessionNote', S.leads.length || S.spend.length ? 'Save from the sidebar' : 'Nothing loaded');

    const state = [
      { what: 'CRM leads', rows: S.leads.length, from: b.leadFrom, to: b.leadTo, detail: S.leads.length ? `${new Set(S.leads.map(l => l.project)).size} projects · ${new Set(S.leads.map(l => l.city)).size} cities · ${new Set(S.leads.map(l => l.assignee)).size} owners` : '—' },
      { what: 'Google spend', rows: g.length, from: g.length ? g.map(x => x.date).sort()[0] : null, to: g.length ? g.map(x => x.date).sort().slice(-1)[0] : null, detail: g.length ? A.money(A.sum(g.map(x => x.cost))) : '—' },
      { what: 'Meta spend', rows: m.length, from: m.length ? m.map(x => x.date).sort()[0] : null, to: m.length ? m.map(x => x.date).sort().slice(-1)[0] : null, detail: m.length ? A.money(A.sum(m.map(x => x.cost))) : '—' }
    ];
  ...
  function renderDrilldown(ctx) {
    ...
    $('#drillKpis').innerHTML = [
      ...
      kpi('Matched to campaigns', A.pct(dd.matchShare, 0), dd.matchShare >= 99.5 ? 'every rupee tied to a project' : dd.matchShare > 0 ? 'the rest is apportioned by lead share' : 'export carries no campaign column', dd.matchShare >= 80 ? '#0E9F6E' : '#D97706'),
  ...
  function bind() {
    ...
    $('#clearLeads').onclick = () => {
      S.leads = [];
      setZoneStatus('leads', 'Cleared', 'bad');
      rebuildFilters(); safeRender();
    };
    $('#clearSpend').onclick = () => {
      S.spend = [];
      setZoneStatus('google', 'Cleared', 'bad');
      setZoneStatus('meta', 'Cleared', 'bad');
      safeRender();
    };
    $('#clearEverything').onclick = () => {
      S.leads = []; S.spend = [];
      ['leads', 'google', 'meta', 'session'].forEach(z => setZoneStatus(z, 'Cleared', 'bad'));
      clearError();
      rebuildFilters(); safeRender();
    };
    ...
    $('#pivotDim').onchange = e => { S.pivotDim = e.target.value; render(); };
    $('#pivotMetric').onchange = e => { S.pivotMetric = e.target.value; render(); };
    $('#pivotTop').onchange = e => { S.pivotTop = Math.max(1, Math.min(20, +e.target.value || 8)); render(); };
    $('#pivotChartType').addEventListener('click', e => {
      const b = e.target.closest('.seg'); if (!b) return;
      $$('#pivotChartType .seg').forEach(x => x.classList.remove('is-active'));
      b.classList.add('is-active'); S.pivotChart = b.dataset.c; render();
    });
