...
(function () {
  ...
  /* ---------- project / dimension trends ---------- */

  C.pivotTrend = function (pv, metric, topN) {
    const def = A.PIVOT_METRICS[metric];
    const rows = pv.rows.slice(0, topN);
    return C.render('chPivotTrend', {
      type: 'line',
      data: {
        labels: pv.labels,
        datasets: rows.map((r, i) => ({
          label: r.key.length > 24 ? r.key.slice(0, 23) + '…' : r.key,
          data: pv.periods.map(p => {
            const v = r.cells.get(p)[metric];
      ...
      options: {
        interaction: { mode: 'index', intersect: false },
        scales: { x: gridX, y: { ...gridY(v => def.short(v)), title: { display: true, text: def.label } } },
        plugins: {
          legend: { position: 'bottom' },
  ...
  C.pivotStack = function (pv, metric, topN) {
    const def = A.PIVOT_METRICS[metric];
    const rows = pv.rows.slice(0, topN);
    return C.render('chPivotStack', {
      type: 'bar',
      data: {
        labels: pv.labels,
        datasets: rows.map((r, i) => ({
          label: r.key.length > 24 ? r.key.slice(0, 23) + '…' : r.key,
          data: pv.periods.map(p => r.cells.get(p)[metric] || 0),
          backgroundColor: C.SERIES[i % C.SERIES.length],
      ...
      options: {
        interaction: { mode: 'index', intersect: false },
        scales: { x: { ...gridX, stacked: true }, y: { ...gridY(v => def.short(v)), stacked: true } },
        plugins: {
          legend: { position: 'bottom' },
          tooltip: { callbacks: { label: c => `${c.dataset.label}: ${def.fmt(c.parsed.y)}` } }
        }
