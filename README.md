...
Open the **Uploads** tab. Four boxes sit in one row, one per file type:
...
Drag a file onto a box, click **Choose file**, or click anywhere in the box to open the picker. Files load on the spot — no staging, no Apply, no platform dropdown. Each box shows what it currently holds in its header and what just happened underneath it: reading, loaded with the row count and total, rejected, or failed with the reason. The two spend exports are structurally identical, so the box you use is what sets the platform; drop one in the wrong box and it is rejected rather than loaded incorrectly.

Anything that goes wrong also raises a red banner at the top of the tab. The collapsed **Diagnostics** strip at the bottom reports whether the CDN libraries loaded, whether the boxes are wired, how many files have been picked, dropped and read, the last event and the last error — open it first if a file ever refuses to load.

Re-uploading is safe. Leads de-duplicate on lead number, spend rows on date plus platform plus account, with the newest file winning. **Loaded data** shows exactly what is in memory, with clear buttons beside it.

**Session files.** The browser holds nothing between visits by design. Press **Save session** in the sidebar to download a JSON snapshot of every lead and spend row, and load it back the next day before adding new files. That is how weekly trends and the audit history build up.

## Optional: auto-load from the repository

Off by default. Set `data-autoload="on"` on the `<html>` tag in `index.html`, then commit the daily CSVs into `data/` alongside a `manifest.json`, and the dashboard loads them on open — no upload step. Left off, the page never requests the manifest, so there is no 404 in the console.
...
Rename `data/manifest.example.json` to `data/manifest.json` and flip the attribute to switch this on. Uploaded files always take precedence over what the manifest loaded.
...
- **Spend Drill-Down** — an expandable project → platform → campaign tree carrying spend, leads, QL, TBD, unqualified, QL rate, CPL, CPQL, visits and EOI on every level. Each row states how it got its spend: `matched`, `apportioned` where it is spread by lead share within the platform, `estimated` where a platform has spend but no tagged leads. Strict mode shows only matched spend and reports the rest as unallocated. The whole tree exports to CSV with the level, project, platform and campaign in separate columns.

## How spend reaches a project

Campaign names in both exports carry the project plus a launch date and a suffix — `Krisala & Hiranandani Pune (10-03-2026)`, `Kolte Patil Vyana Pune-bb`, `Krisala-And-Hiranandani-Hinjewadi-Pune-d-avid`. Substring matching fails on those, so the dashboard scores shared words instead: a campaign belongs to a project when at least three quarters of the project's words appear in the campaign name, longest match wins. Attribution runs in three passes — exact campaign-name match against what the CRM tagged, then the word-score match to a project, then whatever is left apportioned by lead share and labelled as apportioned. On the 21 August exports this ties 100% of spend to a project, so CPL and CPQL per project are real numbers rather than estimates.
...
- **Trends** — daily or weekly granularity, selectable metrics, week-on-week deltas, and cumulative pacing against an even-spend line. Below that, a **breakdown pivot**: pick a dimension (project, platform, city, campaign, source, ASM or pre-sales owner) and a metric (leads, qualified, TBD, spend, CPL, CPQL, qualification rate, visits, dead), and get one row per value with one column per day or week, a window total, an "all" row, and a top-N chart as lines or a stacked bar. Sortable, searchable and exportable. The daily/weekly toggle at the top of the tab drives both halves. Where the metric involves money, spend is matched to the row from the campaign names in the export; anything unmatched is apportioned by that period's lead share, and the card states which applies and for how much.
...
- **Uploads** — the four upload boxes, what is currently loaded, and diagnostics.
...
**Dates are UTC-only.** `new Date('2026-08-01T00:00:00')` parses as local time; calling `toISOString()` on it then shifts the day backwards in any zone east of UTC. In IST that makes `addDays` return the day it was given, which turns a 21-day window into hundreds of copies of one date and multiplies every total. Every date helper in `analytics.js` parses with a trailing `Z` and uses `getUTCDate`/`setUTCDate`. Keep it that way, and note that a UTC container will not reproduce the bug — test with `TZ=Asia/Kolkata`.

## A note on the script files

Each of `parse.js`, `analytics.js`, `audit.js` and `charts.js` is wrapped in its own IIFE. Classic `<script>` tags share one global lexical scope, so a top-level `const PP` in two files is a syntax error that silently kills the second file — the page keeps loading and the failure only shows as a downstream "cannot read properties of undefined". Keep the wrapper when editing, and attach anything shared to `window.PP`.
