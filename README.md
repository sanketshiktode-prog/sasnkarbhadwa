...
Files load the moment they land — no staging step, no Apply button, no platform dropdown to forget. The two spend exports are structurally identical (same headers, same account name), so the zone you drop into is what sets the platform. Drop a file in the wrong zone and it is rejected with a note rather than loaded incorrectly.
